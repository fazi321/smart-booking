          ...
          <section class="Payment">
            ...
            <form class="form">
              <label class="checkbox-container" v-if="dataApi">
                Full Payment (SAR {{ dataApi.totalPrice }})
                <input type="checkbox" />
...
import RequestCard from "@/components/models/RequestCard.vue";
export default {
  name: "payModel",
  props: ["dataApi"],
  components: {
  ...
  computed: {
    ...
    halfPrice: function () {
      if (this.dataApi) {
        var price = this.dataApi.totalPrice;
        if (!price) return "";
  ...
  methods: {
    ...
    async paymentBooking() {
      ...
      const data = {
        amount: this.dataApi.totalPrice,
        currency: "SAR",
        callback_url: `http://localhost:8080/success?bookid=${this.dataApi._id}`,
      };
